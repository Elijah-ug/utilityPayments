...
export default function Profile({ profile, balance }) {
  ...
  //   }, [address]);
  return (
    <Card className="w-md bg-gray-800 border-none text-gray-300 transition-transform duration-300 hover:scale-105 ease-in-out hover:shadow-2xl">
      <CardHeader>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm ">
                {profile?.companyAddr?.slice(0, 7)}...{profile?.companyAddr?.slice(-5)}
              </span>
            </div>

            <div className="flex items-center gap-2">
              <span className="text-lg ">Balance:</span>
              <span className="text-sm ">{`${formatEther(balance.toString())} ETH`}</span>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm ">{profile.utilityService ? profile.utilityService : "N/A"}</span>
            </div>
          </div>
        )}
      </CardContent>
