...
import { useBasePayBlocks } from "@/utils/contractConfig";
...
export function RegisterAndEdit() {
  ...
  const { writeContractAsync: registerCompany, error: registerError, isPending: registerPending } = useWriteContract();
  ...
  const handleAddCompany = async () => {
    ...
    const register = await registerCompany({
      ...
      args: [companyAddr, companyName, utilityService],
    });
        ...
        <TabsContent value="account" className="h-[50vh]">
          <Card className="bg-gray-600 border-none h-full text-white">
            <CardHeader>
              ...
              <CardDescription className="text-white">Register Utility Service Provider</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-6">
              ...
              <div className="grid gap-3">
                ...
                <Input
                  ...
                  id="tabs-demo-utilityService"
                  defaultValue="@peduarte"
                />
              </div>
            </CardContent>
            <CardFooter>
              <Button onClick={handleAddCompany}>{registerPending ? "Registering..." : "Save changes"}</Button>
            </CardFooter>
