          ...
          <form>
            <div className="flex flex-col gap-6">
              <div className="flex items-center gap-2">
                <span>Address:</span>
                <span>
                  {client ? `${client?.client.slice(0, 7)}...${client?.client.slice(-5)}` : 'N/A'}
                </span>
              </div>

              <div className="flex items-center gap-2">
                <span>Balance:</span>
                <span>{client ? `${formatEther(client.balance)} AFB` : '0 AFB'}</span>
              </div>

              <div className="flex items-center gap-2">
                <span>Current Term Payments:</span>
                <span>{client ? `${formatEther(client.termPayments)} AFB`:"0 AFB"}</span>
              </div>
            </div>
          </form>
        </CardContent>
